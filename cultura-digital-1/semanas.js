const semanasCiudadania = [
  {
    numero: 1,
    titulo: "Diagnóstico inicial",
    descripcion: "Responde el formulario para conocer las características del grupo, el acceso a la tecnología y las experiencias previas.",
    tipo: "formulario",
    formulario: {
      titulo: "Responde el diagnóstico aquí",
      indicacion: "Completa todas las preguntas que correspondan a tu situación. Si el formulario no se muestra correctamente, utiliza el botón inferior.",
      incrustado: "https://forms.cloud.microsoft/r/UBwKFCXYiQ?embed=true",
      abrir: "https://forms.cloud.microsoft/r/UBwKFCXYiQ",
      qrImagen: "https://drive.google.com/thumbnail?id=1Y0m31pDeooRq8wmFacHCc2-5YxGOjAen&sz=w1000",
      qrAbrir: "https://drive.google.com/file/d/1Y0m31pDeooRq8wmFacHCc2-5YxGOjAen/view?usp=sharing"
    },
    aviso: "La información se utilizará únicamente con fines académicos para conocer mejor al grupo y orientar la planeación del curso."
  },
  {
    numero: 2,
    titulo: "Del hardware y software a la evolución digital",
    descripcion: "Reconoce cómo trabajan juntos el hardware y el software y analiza cómo la evolución de la tecnología digital ha transformado actividades cotidianas.",
    tipo: "materiales",
    ruta: {
      orden: 1,
      titulo: "Ruta de aprendizaje de la Semana 2",
      descripcion: "Comienza aquí para conocer el propósito, las actividades y el orden de trabajo de la sesión.",
      abrir: "https://drive.google.com/file/d/1-6uTR7_5iPn_XrlqgMZ58fwqPNsYniTC/view?usp=sharing"
    },
    diapositivas: {
      orden: 2,
      titulo: "Del hardware y software a la evolución digital",
      descripcion: "Consulta la presentación para comprender los conceptos centrales y seguir las explicaciones de clase.",
      pdf: "https://drive.google.com/file/d/1371yYoYBbJVQO2a_ezm59fiXXDWQ8uur/preview",
      abrir: "https://drive.google.com/file/d/1371yYoYBbJVQO2a_ezm59fiXXDWQ8uur/view?usp=sharing"
    },
    materiales: [
      {
        orden: 3,
        tipo: "Actividad en clase",
        titulo: "Plantilla del organizador gráfico: Antes y ahora",
        descripcion: "Utiliza esta plantilla para comparar los cambios producidos por la evolución de la tecnología digital.",
        nota: "Importante: imprime esta plantilla y llévala a la clase para realizar la actividad.",
        boton: "Abrir plantilla para imprimir",
        abrir: "https://drive.google.com/file/d/1sPKuoT3MveZ31FolG7ObcBgs2j_DG5Po/view?usp=sharing"
      }
    ]
  },
  {
    numero: 3,
    titulo: "¿De quién es el software?",
    descripcion: "Distingue entre software libre y software gratuito, conoce sus libertades y analiza las opciones que utilizas todos los días.",
    tipo: "materiales",
    ruta: null,
    diapositivas: {
      orden: 1,
      titulo: "Misión: Software Libre",
      descripcion: "Consulta la presentación de la Semana 3 para conocer la historia del software libre, el Proyecto GNU, a Richard Stallman y las cuatro libertades.",
      pdf: "https://drive.google.com/file/d/12IVUO_YFmAwugVZW0UjjodQId35GnEN-/preview",
      abrir: "https://drive.google.com/file/d/12IVUO_YFmAwugVZW0UjjodQId35GnEN-/view?usp=drivesdk"
    },
    materiales: [
      {
        orden: 2,
        tipo: "Actividad colaborativa",
        titulo: "Minilaboratorio: ¿Libre o solo gratis?",
        descripcion: "En equipos, investiguen dos programas y comparen si son gratuitos, si son software libre y qué libertades ofrecen.",
        nota: "Puedes imprimir el minilaboratorio y resolverlo en la hoja, o realizar toda la actividad en tu libreta. En ambos casos, completa la tabla y la conclusión del equipo.",
        boton: "Abrir minilaboratorio",
        abrir: "https://drive.google.com/file/d/1D4iOMgrosXPtfYY3zTws9EQ15XL3QG53/view?usp=drivesdk"
      },
      {
        orden: 3,
        tipo: "Apuntes obligatorios de libreta",
        titulo: "Notas: Software libre",
        descripcion: "Conserva en tu libreta la historia del software libre, el Proyecto GNU, la diferencia entre libre y gratuito y las cuatro libertades.",
        nota: "Puedes imprimir la hoja y pegarla en tu libreta o transcribirla completa. En cualquiera de las dos opciones, responde por escrito todas las reflexiones indicadas.",
        boton: "Abrir notas para libreta",
        abrir: "https://drive.google.com/file/d/1gtU75GhoBvntU7wfm0Ir34zkv97wrsWr/view?usp=drivesdk"
      },
      {
        orden: 4,
        tipo: "Podcast de apoyo",
        titulo: "Podcast de la Semana 3",
        descripcion: "Escucha el episodio para reforzar los conceptos principales de la Semana 3 y diferenciar libertad de gratuidad.",
        boton: "Abrir en Spotify",
        spotify: "https://open.spotify.com/embed/episode/7CElDxIbPg9cz36CqdXYgm",
        abrir: "https://open.spotify.com/episode/7CElDxIbPg9cz36CqdXYgm?si=Ccs4G9dmSiqL5APKYywaCQ"
      }
    ]
  }
];
